...
export function installGlobalSyncSkill({
  ...
  const discoveryLinkExists = validateDiscoveryTarget(discoveryLink, activeSkill);
...
function validateDiscoveryTarget(discoveryLink, activeSkill) {
  if (!existsSync(discoveryLink) && !lstatExists(discoveryLink)) return false;
