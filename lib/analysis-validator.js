...
function validateEvidence(projectRoot, evidence, label, errors) {
  ...
  const path = resolve(projectRoot, evidence.file);
...
function validateRule(rule, context) {
  const { projectRoot, operationById, ruleById, errors } = context;
  ...
  if (!Array.isArray(rule.evidence) || rule.evidence.length === 0) {
    ...
    rule.evidence.forEach((evidence, index) => validateEvidence(projectRoot, evidence, `${rule.id}:${index}`, errors));
  }
}
