...
const REQUIRED_SURFACE_SCANNER_REVISION = 6;
...
function validateSurfaceDiscovery(context) {
  ...
  const usesFullUiCoverageContract = Number.isInteger(discovery.scanner_revision)
    && discovery.scanner_revision >= REQUIRED_SURFACE_SCANNER_REVISION;
  if (!usesFullUiCoverageContract) {
    warnings.push(issue(
      'surface_scanner_revision_outdated',
      'O artefato histórico permanece válido, mas não comprova a cobertura HTML/JavaScript integral do scanner atual.',
      { expected: REQUIRED_SURFACE_SCANNER_REVISION, actual: discovery.scanner_revision ?? null },
    ));
  } else {
    const coverage = Array.isArray(discovery.file_coverage) ? discovery.file_coverage : [];
    const coveredFiles = new Set();
    let coverageInvalid = coverage.length !== discovery.summary?.files_scanned;
    for (const item of coverage) {
      if (!item?.file || coveredFiles.has(item.file)
          || !['analyzed', 'read_error'].includes(item.status)
          || !Number.isInteger(item.candidates) || item.candidates < 0) {
        coverageInvalid = true;
      }
      if (item?.file) coveredFiles.add(item.file);
    }
    if (coverageInvalid) {
      errors.push(issue(
        'surface_file_coverage_invalid',
        'O inventário de arquivos analisados está ausente ou inconsistente.',
      ));
    }
  }
  ...
  for (const candidateId of candidateIds) {
    ...
    if (resolution.disposition === 'auxiliary' && candidate?.type === 'ui_action') {
      const relatedEntryIds = Array.isArray(resolution.related_entry_point_ids)
        ? resolution.related_entry_point_ids
        : [];
      if (usesFullUiCoverageContract
          && candidate.disposition_hint !== 'auxiliary'
          && relatedEntryIds.length === 0) {
        errors.push(issue(
          'functional_ui_auxiliary_unlinked',
          'Controle funcional auxiliar precisa indicar as entradas já catalogadas que absorvem o fluxo.',
          { candidate_id: candidateId },
        ));
      }
      for (const entryId of relatedEntryIds) {
        if (!entryIds.has(entryId)) {
          errors.push(issue(
            'functional_ui_auxiliary_unknown_entry',
            'Controle funcional auxiliar referencia uma entrada inexistente.',
            { candidate_id: candidateId, entry_point_id: entryId },
          ));
        } else if (!referencedEntryIds.has(entryId)) {
          errors.push(issue(
            'functional_ui_auxiliary_unmapped_entry',
            'Controle funcional auxiliar referencia uma entrada sem operação.',
            { candidate_id: candidateId, entry_point_id: entryId },
          ));
        }
      }
