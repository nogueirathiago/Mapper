...
Leia `.reversa/context/surface-candidates.json` antes do inventário. Se o arquivo não existir, tiver versão incompatível ou não corresponder ao código atual, interrompa com falha segura; o orquestrador deve executar `reversa scan-surface --json` antes de ativar ou reativar o Scout. Para artefatos da revisão atual do scanner, confira `file_coverage`: cada arquivo precisa estar `analyzed` ou registrado em `scan_gaps`. Artefatos históricos permanecem válidos no contrato em que foram aprovados e recebem aviso de cobertura antiga; não os reclassifique apenas para adotar campos novos. Processe os candidatos em lotes por arquivo ou módulo, sem carregar todo o corpus no contexto de uma vez.

O scanner fornece somente fatos sintáticos. Você é responsável pela classificação semântica: crie exatamente uma resolução por candidato em `surface.json.surface_discovery.candidate_resolutions`, usando apenas `promoted`, `auxiliary`, `excluded` ou `pending`. Promova somente efeitos funcionais observáveis e vincule cada promoção a um ID estável de `operation_entry_points`. No contrato da revisão atual, um `ui_action` funcional só pode ser `auxiliary` quando o mesmo fluxo já estiver representado: registre todas as entradas absorventes em `related_entry_point_ids`. A exceção são controles comprovadamente visuais marcados pelo scanner com `disposition_hint: "auxiliary"`. Resoluções históricas sem esse campo continuam válidas. Justifique `auxiliary` e `excluded`; quando a evidência não bastar, use `pending` sem inventar destino nem descartar o candidato.
