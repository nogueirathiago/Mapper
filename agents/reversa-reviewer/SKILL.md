...
3. Leia `../reversa/references/behavioral-analysis-guide.md`. Use `.reversa/context/surface-candidates.json`, `.reversa/context/surface.json` e `.reversa/context/modules.json` como índices para a triagem incremental do escopo solicitado. Compare primeiro o snapshot, o `file_coverage` quando fornecido pela revisão atual, os IDs e as evidências; selecione as operações/units que exigem aprofundamento pelos critérios do guia. Para `ui_action` funcional classificado como auxiliar no contrato atual, confira `related_entry_point_ids` e as operações vinculadas. Artefatos históricos permanecem avaliados pelo contrato em que foram aprovados. Um pedido explícito para reconferir fontes já analisadas inclui também os itens reaproveitáveis, somente no recorte indicado.
